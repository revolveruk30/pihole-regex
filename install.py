...
url_regexps_remote = 'https://raw.githubusercontent.com/revolveruk30/pihole-regex/master/regex.list'
install_comment = 'github.com/revolveruk30/pihole-regex'
...
regexps_remote = set()
regexps_local = set()
regexps_revolveruk30_local = set()
regexps_legacy_revolveruk30 = set()
regexps_remove = set()
...
if db_exists:
    ...
    # Fetch all current revolveruk30 regexps in the local db
    c.execute('SELECT domain FROM domainlist WHERE type = 3 AND comment = ?', (install_comment,))
    regexps_revolveruk30_local_results = c.fetchall()
    regexps_revolveruk30_local.update([x[0] for x in regexps_revolveruk30_local_results])
    ...
    regexps_remove = regexps_revolveruk30_local.difference(regexps_remote)
    ...
    # Delete revolveruk30-regex.list as if we've migrated to the db, it's no longer needed
    if os.path.exists(path_legacy_revolveruk30_regex):
        os.remove(path_legacy_revolveruk30_regex)
    ...
    if regexps_local:
        ...
        if os.path.isfile(path_legacy_revolveruk30_regex) and os.path.getsize(path_legacy_revolveruk30_regex) > 0:
            print('[i] Existing revolveruk30-regex install identified')
            # Read the previously installed regexps to a set
            with open(path_legacy_revolveruk30_regex, 'r') as fOpen:
                regexps_legacy_revolveruk30.update(x for x in map(str.strip, fOpen) if x and x[:1] != '#')

                if regexps_legacy_revolveruk30:
                    print('[i] Removing previously installed regexps')
                    regexps_local.difference_update(regexps_legacy_revolveruk30)
    ...
    # Output revolveruk30 remote regexps to revolveruk30-regex.list
    # for future install / uninstall
    with open(path_legacy_revolveruk30_regex, 'w') as fWrite:
